...
// TestAPIValidator_ValidateSoapOperations covers SOAP operation uniqueness rules,
// including the bodyElement (document/literal wrapper) dispatch field.
func TestAPIValidator_ValidateSoapOperations(t *testing.T) {
	v := NewAPIValidator()
	...
	tests := []struct {
		name      string
		ops       []api.SoapOperation
		wantField string // "" = expect no error
	}{
		{
			name: "Valid distinct operations",
			ops: []api.SoapOperation{
				{Name: "Add", SoapAction: strPtr("urn:Add")},
				{Name: "getQuote", SoapAction: strPtr(""), BodyElement: strPtr("GetQuoteRequest")},
			},
		},
		{
			name: "Duplicate non-empty bodyElement",
			ops: []api.SoapOperation{
				{Name: "opA", BodyElement: strPtr("SameWrapper")},
				{Name: "opB", BodyElement: strPtr("SameWrapper")},
			},
			wantField: "spec.operations[1].bodyElement",
		},
		{
			name: "Empty bodyElements are not duplicates",
			ops: []api.SoapOperation{
				{Name: "opA", SoapAction: strPtr("urn:a")},
				{Name: "opB", SoapAction: strPtr("urn:b")},
			},
		},
		{
			name: "Duplicate soapAction still caught",
			ops: []api.SoapOperation{
				{Name: "opA", SoapAction: strPtr("urn:same")},
				{Name: "opB", SoapAction: strPtr("urn:same")},
			},
			wantField: "spec.operations[1].soapAction",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ops := tt.ops
			errs := v.validateSoapOperations(&ops)
			found := false
			for _, e := range errs {
