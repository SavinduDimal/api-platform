...
func (v *APIValidator) validateSoapData(spec *api.SoapAPIData) []ValidationError {
	...
	errors = append(errors, v.validateSoapOperations(spec.Operations)...)

	return errors
}

// validateSoapOperations validates the operations list of a SOAP API.
func (v *APIValidator) validateSoapOperations(operations *[]api.SoapOperation) []ValidationError {
	var errors []ValidationError
	if operations == nil {
		return errors
	}
	...
	for i, op := range *operations {
		if op.Name == "" {
			errors = append(errors, ValidationError{
				Field:   fmt.Sprintf("spec.operations[%d].name", i),
				Message: "Operation name is required",
			})
			continue
		}

		if namesSeen[op.Name] {
			errors = append(errors, ValidationError{
				Field:   fmt.Sprintf("spec.operations[%d].name", i),
				Message: fmt.Sprintf("Duplicate operation name '%s'", op.Name),
			})
		}
		namesSeen[op.Name] = true

		// Multiple operations with an empty soapAction are valid (doc/literal);
		// only flag duplicates for non-empty soapActions since those generate distinct routes.
		if op.SoapAction != nil && *op.SoapAction != "" {
			if actionsSeen[*op.SoapAction] {
				errors = append(errors, ValidationError{
					Field:   fmt.Sprintf("spec.operations[%d].soapAction", i),
					Message: fmt.Sprintf("Duplicate soapAction '%s'; each non-empty soapAction must be unique", *op.SoapAction),
				})
			}
